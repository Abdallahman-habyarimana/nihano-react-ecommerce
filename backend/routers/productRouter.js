...
productRouter.post('/', isAuth, isSellerOrAdmin, asyncHandler(async(req, res) => {
...
productRouter.put("/:id", isAuth, isSellerOrAdmin,  asyncHandler(async(req, res) => {
...
productRouter.post("/:id/reviews", isAuth, asyncHandler(async(req, res) => {
    ...
    if(product){
        if(product.reviews.find(x => x.name === req.user.name)) {
            return res.status(400).send({ message: 'Already submitted a review'})
        }
        const review = {
            name: req.user.name, 
